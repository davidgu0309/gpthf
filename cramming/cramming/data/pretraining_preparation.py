...
def load_pretraining_corpus(cfg_data, cfg_impl):
    ...
    elif list(cfg_data.sources.values())[0]["provider"] == "thf":
        if cfg_data.name == "thf":
            tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")
            is_debug = cfg_data.debug
            mode = getattr(cfg_data, 'mode', None)
        elif cfg_data.name == "gpthf":
            ...
            is_debug = cfg_data.debug
            mode = getattr(cfg_data, 'mode', None)
        return _load_thf_dataset(tokenizer, is_debug, context_length=cfg_data.seq_length, mode=mode), tokenizer
    else:
        try:
...
def _load_thf_dataset(tokenizer, is_debug, context_length, mode=None):
    if is_debug:
        #base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../../data/piece128')
        base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data')
        combined_dataset = datasets.load_dataset('text', data_files=os.path.join(base_path, 'shard-0.txt'), split='train', cache_dir=base_path)
        train_dataset = combined_dataset['train'] if 'train' in combined_dataset else combined_dataset
        train_dataset.set_transform(lambda row: tokenize_and_sentenize(tokenizer, row, max_size=context_length, is_generate=False))
        return train_dataset
    else:
        if context_length == 128:
            base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../../data/piece128')
            combined_dataset = datasets.load_dataset('text', data_files=[ os.path.join(base_path, f'bert_dataset_shard{i}_pieced.txt') for i in range(200)], split='train', cache_dir=base_path)
            train_dataset = combined_dataset['train'] if 'train' in combined_dataset else combined_dataset
            train_dataset.set_transform(lambda row: tokenize_and_sentenize(tokenizer, row, max_size=context_length, is_generate=False))
            return train_dataset
        else:
            if mode == 'pile':
                base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data/pile_dedup_{context_length}')
            elif mode == 'newdata':
                #base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data/newdata_{context_length}')
                base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data/newdata_piece_{context_length}')
            elif mode == 'c4':
                base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data/c4_{context_length}')
            else:
                base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), f'../../../data/fineweb_{context_length}')
                
            combined_dataset = datasets.load_dataset('text', data_files=[os.path.join(base_path, f'shard-{i}.txt') for i in range(80)], split='train', cache_dir=base_path)
        print(f"Loaded dataset with context length {context_length}")
        
    train_dataset = combined_dataset['train'] if 'train' in combined_dataset else combined_dataset
    train_dataset.set_transform(lambda row: tokenize_and_sentenize(tokenizer, row, max_size=context_length))
    return train_dataset

def tokenize_and_sentenize(tokenizer, row, max_size=1024, is_generate=True):
    ...
    for text_entry in row['text']:
        ...
        # if len(input_ids) < 500:   #not worth it to use resources on a sequence that will have more than half padding tokens
        #     continue

        all_input_ids.append(input_ids)
        all_attention_masks.append(attention_masks)
        all_sentence_ids.append(sentence_ids)
        # all_sentence_ids.append(range(len(sentence_ids)))
