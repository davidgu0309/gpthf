...
from .thf_utils import generate_block_matrix, last_sentence_indices, fetch_last_sentence_embeddings, create_column_mask, combine_attention_masks, truncate_tensor, prepare_inputs_for_generation, create_overwrite_mask, overwrite_with_mask
...
class ScriptableTHFLM(PreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask, sentence_ids=None, position_ids=None):
        sentence_embeddings, sentence_indices, sentence_attention_mask, block_mask, word_triangular_mask = self.forward_encoder(input_ids, attention_mask, sentence_ids, is_triangular=True, use_cache=False, position_ids=position_ids)
    ...
    def forward_encoder(self, input_ids, attention_mask, sentence_ids=None, is_triangular=False, use_cache=False, position_ids=None):   
...
class ScriptableLMForPreTraining(PreTrainedModel):
    ...
    def generate(self, idx, attention_mask, sentence_ids, max_new_tokens, temperature=1.0, do_sample=False, top_k=None, use_cache=True):
        ...
        for i in range(max_new_tokens):
            ...
            embeddings, sentence_indices, sentence_attention_mask_candidate, _, _ = self.encoder.forward_encoder(idx, attention_mask,
                            sentence_ids, is_triangular=False, use_cache=use_cache, position_ids=position_ids)
    ...
    @torch.no_grad()
    def generate_fast(self, idx, attention_mask, sentence_ids, max_new_tokens, temperature=1.0, do_sample=False, top_k=None):
        ...
        for i in range(max_new_tokens):
            ...
            embeddings, sentence_indices, sentence_attention_mask, _, _ = self.encoder.forward_encoder(idx, attention_mask, sentence_ids, is_triangular=False, position_ids=position_ids)
            ...
            sentence_body_input = sentence_embedding_cache.clone()[:, :max_current_sentences]
            
            overwrite_mask = create_overwrite_mask(num_cached_sentences, num_cached_sentences+num_new_sentences, max_current_sentences)
            sentence_body_input = overwrite_with_mask(sentence_body_input, sentence_embeddings[POSITION_IDS[:, :sentence_embeddings.size(1)] < num_new_sentences.unsqueeze(1)], overwrite_mask)           
            ...
            for layer in self.encoder.slt_body:
                sentence_body_input = layer(sentence_body_input, sentence_attention_mask_float, position_ids=range_tensor, is_sentence_end=is_eos.any())[0]
            ...
            if is_eos.any():
                ...
                idx, attention_mask, sentence_ids = truncate_tensor(idx, sentence_ids, pad_token_id), truncate_tensor(attention_mask, sentence_ids, 0), truncate_tensor(sentence_ids, sentence_ids, -1, normalize=True)
