...
def initialize_torch(model, dataset, tokenizer, cfg_train, cfg_impl, elapsed_time, setup=_default_setup, is_thf=False, is_generate=False):
    """initialize a torch engine."""
    if is_generate:
        train_dataloader, val_dataloader = None, None
    elif dataset is not None:
        train_dataloader, val_dataloader = prepare_pretraining_dataloader(dataset, tokenizer, cfg_train, cfg_impl, is_thf=is_thf)
    else:
        train_dataloader, val_dataloader = None, None
    ...
    if require_full_engine:
        model_engine = TorchEngineFull(model, cfg_train, cfg_impl, elapsed_time, seq_length=tokenizer.model_max_length, is_generate=is_generate)
    else:
        model_engine = TorchEngineMinimal(model, cfg_train, cfg_impl, elapsed_time, setup=setup, seq_length=tokenizer.model_max_length, is_generate=is_generate)
    model_engine.train()  # This is the default engine state. Pretraining scripts may change this.

    if is_generate:
        return model_engine, None, None, train_dataloader, None
    
    return model_engine, model_engine.optimizer, model_engine.scheduler, train_dataloader, val_dataloader
...
class TorchEngineMinimal(torch.nn.Module):
    ...
    def __init__(self, model, cfg_train, cfg_impl, already_elapsed_time=0.0, setup=_default_setup, seq_length=128, is_generate=False):
        ...
        self.current_seq_length = cfg_train.max_seq_length if cfg_train.max_seq_length is not None else seq_length
    ...
    def validation_step(self, batch: dict[str, torch.Tensor]):
        with torch.no_grad():
            return self.forward(**batch)["loss"].detach()

    def to_device(self, batch: dict[str, torch.Tensor], keys: list[str] = ["input_ids", "attention_mask", "sentence_ids", "labels"]):
    ...
    def save_training_checkpoint(self, identifier, directory="", metadata=None):
    ...
    def load_training_checkpoint(self, identifier="intermediate.pth", directory=""):
        ...
        #this is actually caused by just a version error, for some versions we have to remove the module. prefix if they exist
...
class TorchEngineFull(TorchEngineMinimal):
    ...
    def __init__(self, model, cfg_train, cfg_impl, setup=_default_setup, seq_length=128, is_generate=False):
        """Load Engine. The model will be compiled by default."""
        ...
        self.data_seq_length = cfg_train.max_seq_length#seq_length
        self.current_seq_length = cfg_train.max_seq_length if not self.sequence_curriculum else cfg_train.sequence_curriculum.lengths[0]
