...
def load_backend(model, dataset, tokenizer, cfg_train, cfg_impl, elapsed_time=0.0, setup=_default_setup , is_thf=False, is_generate=False):
    if cfg_impl.name == "torch-default":
        return initialize_torch(model, dataset, tokenizer, cfg_train, cfg_impl, elapsed_time, setup=setup, is_thf=is_thf, is_generate=is_generate)
