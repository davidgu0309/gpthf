...
def prepare_task_dataloaders(tokenizer, cfg_eval, cfg_impl, is_thf=False):
    ...
    for task_name, task_details in cfg_eval.tasks.items():
        ...
        def preprocess_function(examples):
            ...
            if len(task_details.structure) == 2:
                ...
                texts = tuple(examples[sentence_key] for sentence_key in task_details.structure)
                premises = [tokenizer.pad_token.join([str(f) for f in fragments]) for fragments in zip(*texts[:-1])]  # this should be sep_token whenever possible!!!
                if isinstance(texts[-1][0], list):
                    # merge multiple hypotheses
                    hypothesis = [tokenizer.pad_token.join([f"{l}:{a}" for l, a in zip(label_list, answers)]) for answers in texts[-1]]
                else:
                    hypothesis = texts[-1]
            ...
            for text1, text2 in zip(texts[0], texts[1]):
                tokenized_pair = tokenizer(text1, text2, truncation=True, max_length=max_seq_length)
                all_input_ids1.append(tokenized_pair['input_ids'])
                all_attention_masks1.append(tokenized_pair['attention_mask'])
                l = len(tokenizer(text1, truncation=True, max_length=max_seq_length)['input_ids'])
                all_sentence_ids1.append([0] * l + [0] * (len(tokenized_pair['input_ids']) - l))
                #all_sentence_ids1.append([0] * l + [1] * (len(tokenized_pair['input_ids']) - l))

                    
            result = {
                "input_ids": all_input_ids1,
                "attention_mask": all_attention_masks1,
                "sentence_ids": all_sentence_ids1,
            }

            # assert result['input_ids'] == result2['input_ids']
            # assert result['attention_mask'] == result2['attention_mask']
...
def _build_dataloaders(tokenizer, train_dataset, eval_dataset, extra_eval_dataset, cfg_impl, is_thf=False, seq_length=None):
    """Construct dataloaders according to cfg_impl settings. Validation samplers always repeat on all devices."""
    train_dataloader = prepare_downstream_dataloader(train_dataset, tokenizer, "training", cfg_impl, is_thf=is_thf)
    eval_dataloader = prepare_downstream_dataloader(eval_dataset, tokenizer, "eval", cfg_impl, is_thf=is_thf, seq_length=seq_length)
    if extra_eval_dataset is not None:
        extra_eval_dataloader = prepare_downstream_dataloader(extra_eval_dataset, tokenizer, "eval", cfg_impl, is_thf=is_thf)
    else:
