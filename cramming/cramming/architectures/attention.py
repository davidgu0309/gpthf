...
def get_attention_mechanism(
    ...
    if cfg_attention.type == "self-attention":
        mechanism = LegacySeqFirstSelfAttention(hidden_size, cfg_attention)  # neox
    elif cfg_attention.type == "batch-first-attention":
        mechanism = BatchFirstSelfAttention(hidden_size, cfg_attention)
...
class SelfAttentionPyTorch(torch.nn.Module):
    ...
    def forward(self, hidden_states, attention_mask: Optional[torch.Tensor] = None, key_padding_mask: Optional[torch.Tensor] = None, is_causal=False):
        return self.attn(hidden_states, hidden_states, hidden_states, need_weights=True, attn_mask=attention_mask, key_padding_mask=key_padding_mask, is_causal=is_causal)[0]
...
class BatchFirstSelfAttention(LegacySeqFirstSelfAttention):
    """Self-attention layer.
    ...
    def attention(self, query_layer, key_layer, value_layer, attention_mask: Optional[torch.Tensor] = None, training: bool = False):
        B, S, NP, HN = query_layer.shape
        query_layer = query_layer.view(B * NP, S, HN)
        key_layer = key_layer.view(B * NP, S, HN)

        matmul_result = torch.bmm(query_layer, key_layer.transpose(1, 2)) * self.norm_factor
        attention_scores = matmul_result.view(B, NP, S, S)
        attention_probs = self.sequence_op(attention_scores, attention_mask)
        attention_probs = torch.nn.functional.dropout(attention_probs, p=self.attention_dropout, training=training)

        value_layer = value_layer.view(B * NP, S, HN)
        attention_probs = attention_probs.view(B * NP, S, S)

        context_layer = torch.bmm(attention_probs, value_layer)

        context_layer = context_layer.view(B, NP, S, HN)
        return context_layer
...
class TorchSoftmax(torch.nn.Module):
    ...
    def forward(self, inputs, attention_mask: Optional[torch.Tensor] = None):
        ...
        if attention_mask is not None:
            # if attention mask is bool where false denotes keep and true denotes setting to -inf
            if attention_mask.dtype == torch.bool:
                inputs[attention_mask] = float("-inf")
            else:
