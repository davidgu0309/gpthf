...
from .components import _init_module, PredictionHeadComponent, PoolingComponent
...
class ScriptableFunnelLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.num_transformer_layers = len(self.cfg.setup)
        layers = []
        seq_length_in = self.cfg.setup[0]
        for idx, layer_spec in enumerate(self.cfg.setup[1:]):
            seq_length_out = layer_spec        
            layers.append(torch.jit.script(FunnelLayer(self.cfg, seq_length_in, seq_length_out)))
            seq_length_in = layer_spec
        self.cutoff: int = torch.as_tensor(self.cfg.setup).argmin().item() - 1
    ...
    def forward(self, input_ids, attention_mask: Optional[torch.Tensor] = None, labels: Optional[torch.Tensor] = None, **kwargs):
        ...
        for i, layer_module in enumerate(self.layers):
           ...
           # print("Layer ", i, " ", hidden_states.shape)
